...
% dp = model.operator.grad(p_ad);
  ...
  q_w = (2*pi*rW(model.well.prodIndex)*model.rock.perm(model.well.prodIndex)*mobW(model.well.prodIndex)*model.G.cells.centroids(model.well.prodIndex,3)) ...
    *(200*barsa - p_ad(model.well.prodIndex).val);
  q_o = (2*pi*rO(model.well.prodIndex)*model.rock.perm(model.well.prodIndex)*mobO(model.well.prodIndex)*model.G.cells.centroids(model.well.prodIndex,3)) ...
    *(200*barsa - p_ad(model.well.prodIndex).val);
